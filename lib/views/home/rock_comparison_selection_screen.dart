...
class _RockFirstSelectionScreenState extends State<RockFirstSelectionScreen> {
  ...
  Future<void> fetchStones() async {
    final snapshot =
        await FirebaseFirestore.instance.collection('_rocks').limit(50).get();
  ...
  Widget build(BuildContext context) {
    final filteredStones = stones.where((stone) {
      final name = (stone["tenDa"] ?? "").toString().toLowerCase();
  ...
  Widget _buildStoneCard(Map<String, dynamic> stone) {
    ...
    final image = (stone["hinhAnh"] is List && stone["hinhAnh"].length > 0)
        ? stone["hinhAnh"][0]
        : (stone["hinhAnh"] ?? 'assets/placeholder.jpg');
